...
hour_resolution = ARGV[0] ? ARGV[0].to_i : 5
...
File.open(infile).each do |line|
	...
	# if diff < 0 || diff > hour_resolution
	# 	of << "diff:" << diff.to_s << "\t\t" << period[1] << " " << period[2] << ":" << period[3] << ":" << $4 << "," << open << "," << high << "," << low << "," << close << "\n"
	# 	period = current_period
	# end

	if period != current_period && current_period[2].to_i % hour_resolution == 0 && current_period[3].to_i == 0
